...
void proband(int b1, int atom_num, int bnd_num, kpoint_num){
 FILE *f1;                              //up_band
 FILE *f2;                              //dn_band
 FILE *f3;                              //PROCAR
 FILE *f4;                              //kpoint-path
...
void gnuband(FILE *filename,char *material_name,double fermi_level,int bnd_num,double mag,double min_dos,double DOS_unit){
 ...
 double i;
  ...
  filename=fopen("./gnuband", "wt");
   ...
   fprintf(filename,"plot    \"up_band\" using ($1):($2+%lf) w l  lw 4 lt 1 lc 0 ,\\\n",fermi_level*-1);
   fprintf(filename,"#plot    \"dn_band\" using ($1):($2+%lf) w l  lw 4 lt 1 lc 1 ,\\\n",fermi_level);
  fclose(filename);
